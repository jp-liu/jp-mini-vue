...
function createRootCodegenNode(root, context) {
    const { children } = root;
    const firstChild = children[0];
    if (firstChild) {
        if (firstChild.type === 3 /* ELEMENT */) {
...
function isText(node) {
    return node.type === 4 /* TEXT */ || node.type === 1 /* INTERPOLATION */;
}
...
class ReactiveEffect {
    constructor(fn, scheduler) {
        this.deps = [];
        this.active = true; // 是否被执行了 `stop` 退出了响应式系统的收集
        this._fn = fn;
...
function flushJobs() {
    ...
    while ((job = queue.shift())) {
        console.log(job === null || job === void 0 ? void 0 : job.component);
        job && job();
    }
}
...
function createRenderer(options) {
    // 提供自定义渲染接口
    // 起别名
    const { createElement: hostCreateElement, createTextNode: hostCreateText, patchProp: hostPatchProp, insert: hostInsert, remove: hostRemove, setElementText: hostSetElementText } = options;
    // 提供给外部的启动渲染方法
...
export { createApp, createElementVNode, createRenderer, createTextVnode, getCurrentInstance, h, inject, nextTick, provide, reactive, ref, registerRuntimeComplier, renderSlot, toDisplayString };
